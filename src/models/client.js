...
async function createClient(client) {
  if (
    val.isEmptry(client.name) ||
    val.isEmptry(client.email) ||
    val.isEmptry(client.address) ||
    val.isEmail(client.email)
  ) {
...
async function updateClientAddress(client, address) {
  if (val.isEmpty(address)) {
    return {
      text: "Looks like there is no Event Location. Please Provide address of venue",
    };
  }
  try {
    const query = {
