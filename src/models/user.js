...
async function createUser(user) {
    ...
    let res = ''
    if (val.isEmpty(user.name) || val.isEmpty(user.email) || val.isEmpty(user.password)) {

        return res = { message: 'Please fill all require fields' }
    }
    if (user.password.length < 3) {
        return res = { message: 'Please use password at least with 4 digits/symbols' }
    }
    if (!val.isEmail(user.email)) {
        return res = { message: 'Im not sure that this is Email. Please Check' }
    }
    const users = await db.query('select * from users where email=$1', [user.email])
...
async function updateUserPassword(user, oldPassword, newPassword) {
    ...
    const pwd = await db.query('select password from users where id=$1', [user.id])
    ...
    try {
        ...
        if (res.rowCount === 1) {
            ...
            return { text: 'Oops! Looks like an problem here. Try again one more time' }
        }
...
async function deleteUser(user) {
    if (user.id === undefined) {
        return { text: 'user is undefined. please Check everythings one more time.' }
    }
    try {
        const query = {
...
async function generateAuthToken(user) {
    ...
    const updateUser = await db.query('update users set tokens=$1 where id=$2 returning id,name,email,tokens', ['{' + token + '}', user.id])
